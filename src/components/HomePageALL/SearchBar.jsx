...
const locations = ['Online', 'Physical'];
...
export default function SearchBar() {
  ...
  const handleSearch = () => {
    ...
    if (!searchValue && !locationValue) {
      alert('Please select a subject and location to search');
      return;
    }
    if (!searchValue) {
    ...
    if (!locationValue) {
      alert('Please select a location (Online or Physical)');
      return;
    }

    // Search is successful - you can add your actual search logic here
    alert(`Searching for ${searchValue} tutors - ${locationValue}`);
  };
          ...
          <div className="bg-white rounded-full p-1 sm:p-2 flex items-center shadow-lg">
            ...
            <div className="flex items-center flex-1 px-2 sm:px-4 py-2 sm:py-3 border-r border-gray-200 relative min-w-0" ref={subjectRef}>
              ...
              <div className="flex-1 relative min-w-0">
                ...
                />
                <ChevronDown
                  className="absolute right-0 top-1/2 transform -translate-y-1/2 h-3 w-3 sm:h-4 sm:w-4 text-gray-400 cursor-pointer flex-shrink-0"
                  onClick={handleSubjectInputClick}
                />
                ...
                {showSubjectDropdown && (
                  <div className="absolute top-full left-0 right-0 mt-2 bg-white rounded-2xl shadow-lg border border-gray-100 max-h-60 overflow-y-auto z-20">
                    ...
                    {filteredSubjects.length === 0 && searchValue && (
                      <div className="px-3 sm:px-4 py-2 sm:py-3 text-gray-500 text-center text-sm sm:text-base">
                        No subjects found
                      </div>
                    )}
                  </div>
